def compute_skill_score(jd: dict, candidate: dict, weights: dict = None) -> dict:
    ...
    if required:
        matched_required = [s for s in required if any(s in cs or cs in s for cs in candidate_skills)]
    ...
    if preferred:
        matched_preferred = [s for s in preferred if any(s in cs or cs in s for cs in candidate_skills)]
